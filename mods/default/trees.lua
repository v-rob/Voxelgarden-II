...
local c_cherry_leaves = minetest.get_content_id("default:cherry_leaves")
...
function default.grow_tree(pos, is_apple_tree, bad)
	...
	add_trunk_and_leaves(data, a, pos, c_tree, c_leaves, height, 2, 6, is_apple_tree)
...
function default.grow_cherry_tree(pos, is_apple_tree, bad)
	...
	add_trunk_and_leaves(data, a, pos, c_tree, c_cherry_leaves, height, 2, 6)
...
function default.grow_cherry_sapling(pos)
	if not default.can_grow(pos) then return true end
	if minetest.find_node_near(pos, 1, {"group:tree", "group:sapling"}) then
		minetest.set_node(pos, {name="default:grass_"..math.random(1, 5)})
		return
	...
	default.grow_cherry_tree(pos, math.random(1, 8) == 1)
end
