	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if dest ~= nil then
			if returnmirror.mana_check(user, returnmirror.cost_teleport) then
				fail = false
				minetest.sound_play( {name="returnmirror_teleport1", gain=1}, {pos=src, max_hear_distance=12})
				user:setpos(dest)
				minetest.after(0.5, function(dest) minetest.sound_play( {name="returnmirror_teleport2", gain=1}, {pos=dest, max_hear_distance=12}) end, dest)
			end
		end
		if fail then
